	...
	while(i < fmt.length) {
		switch((c = fmt.charAt(i))) {
			...
			case 'A': case 'a': case 'ä¸Š':
				...
				if(fmt.substr(i, 5).toUpperCase() === "ä¸Šåˆ/ä¸‹åˆ") return true;
				++i; break;
			case '[':
				o = c;
	...
	while(i < fmt.length) {
		switch((c = fmt.charAt(i))) {
			...
			case 'A': case 'a': case 'ä¸Š':
				...
				else if(fmt.substr(i,5).toUpperCase() === "ä¸Šåˆ/ä¸‹åˆ") { if(dt!=null) q.v = dt.H >= 12 ? "ä¸‹åˆ" : "ä¸Šåˆ"; q.t = 'T'; i+=5; hr='h'; }
				else { q.t = "t"; ++i; }
...
function cc2str(arr/*:Array<number>*/, debomit)/*:string*/ {
	...
	if(typeof TextDecoder !== "undefined") try {
		...
		return new TextDecoder("latin1").decode(arr).replace(/[â‚¬â€šÆ’â€žâ€¦â€ â€¡Ë†â€°Å â€¹Å’Å½â€˜â€™â€œâ€â€¢â€“â€”Ëœâ„¢Å¡â€ºÅ“Å¾Å¸]/g, function(c) { return rev[c] || c; });
	} catch(e) {}
